...
nombre_proie=haut*20
nombre_predateur=haut*20
nombre_plante=haut*20
...
plt.figure(1)
plt.subplot(211)
plt.plot(temps,proietemps, color='blue')
plt.plot(temps,predateurtemps, color='red')
plt.axis([0, max(temps), 0,max(predateurtemps+proietemps)])

plt.subplot(212)
for i in range (0,len(proietemps)-1,25):
    l=[]
    u=[]
    for j in range(25):
        l.append(proietemps[i+j])
        u.append(predateurtemps[i+j])
    plt.plot(l,u, color='orange',alpha = 0.3)

plt.axis([0, max(proietemps)*1.5, 0,max(predateurtemps)*1.5])
plt.show()
